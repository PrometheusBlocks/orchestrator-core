...
def test_load_specs(tmp_path, monkeypatch):
    ...
    specs_data = [
        ...
        {"name": "baz", "version": "invalid-version", "description": "baz invalid"}, # Should be skipped
        {"name": "qux-1.0.0", "version": "1.0.0", "description": "qux v1"}, # Test filename parsing
        {"name": "corge", "version": "1.0"}, # No hyphen in filename stem, should be skipped if named corge.json
    ]
    ...
    (registry_dir / "corge.json").write_text(json.dumps({"name": "corge", "version": "1.0"}))
    ...
    assert "corge" not in result # Should be skipped due to missing hyphen in filename stem
